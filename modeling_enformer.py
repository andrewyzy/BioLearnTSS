...
def ConvBlock(dim, dim_out = None, kernel_size = 1, is_distributed = None):
    ...
    return nn.Sequential(
        ...
        SqueezeExcitation(default(dim_out, dim)) if use_se else nn.Identity()
    )
...
class Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.use_learned_abs_pos = use_learned_abs_pos
        if use_learned_abs_pos:
            self.abs_pos_embed = nn.Parameter(torch.randn(heads, 1, dim_key))
    ...
    def forward(self, x):
        ...
        if self.use_learned_abs_pos:
            abs_pos_logits = einsum('b h i d, h j d -> b h i j', q, self.abs_pos_embed)
            logits = content_logits + rel_logits + abs_pos_logits
        else:
